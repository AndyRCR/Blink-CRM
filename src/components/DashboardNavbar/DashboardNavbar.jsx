...
const obtainTranslateX = (el) => {
	return parseInt(
		el.style.transform.replace('translateX(', '').replace(')', '')
	)
}
...
const DashboardNavbar = () => {
	...
	const handleMenu = () => {
		if (windowHeight <= 900)
			setResponsiveMenuDisplayed(!responsiveMenuDisplayed)
		else {
			setMenuState(!menuState)
			if (menuState) {
				document.querySelectorAll('.sliderItem').forEach((el) => {
					const translate =
						obtainTranslateX(el) === 0
							? 0
							: obtainTranslateX(el) + 170
	...
	return (
		<div className='dashboardNavbar'>
			<div className='logoContainer'>
				...
				<div className='logo'>
					<img
						src='https://i.ibb.co/DtZnB6y/blinklogo.png'
						alt='blink logo'
					/>
				</div>
			</div>
